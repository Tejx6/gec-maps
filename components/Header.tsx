...
const Header: React.FC<HeaderProps> = ({ theme, toggleTheme, searchQuery, onSearchChange, searchResults, onSearchResultClick, isAddingLocation, onToggleAddLocationMode, isAddingEvent, onToggleAddEventMode, onManageEventsClick, onToggleViewMode, user }) => {
    return (
        <header className="absolute top-0 left-0 right-0 z-20 grid grid-cols-3 items-center p-4 gap-4">
            <div className="flex items-center gap-4 col-span-1 justify-self-start">
                <div className="px-4 py-2 rounded-lg bg-black/20 backdrop-blur-md shadow-lg border border-white/10">
                    <h1 className="text-xl font-bold tracking-wider text-white uppercase drop-shadow-[0_1px_2px_rgba(0,0,0,0.5)]">
                ...
                {user?.type === 'admin' && (
                    <>
                        <button 
                            ...
                            className={`px-4 py-2 rounded-lg backdrop-blur-md shadow-lg flex items-center gap-2 transition-all duration-300 text-white border ${
                                isAddingLocation 
                ...
                {(isAddingLocation || isAddingEvent) && user?.type === 'admin' && (
                    <p className="text-sm font-medium text-cyan-200 bg-black/30 px-3 py-1 rounded-full animate-pulse">
                        Click on the map to place a new point
                    </p>
                )}
            </div>

            <div className="w-full max-w-md col-span-1 justify-self-center">
            ...
            <div className="flex items-center gap-4 col-span-1 justify-self-end">
                {user?.type === 'admin' && (
                    <button 
                        onClick={onManageEventsClick}
                        className="px-4 py-2 rounded-lg backdrop-blur-md shadow-lg flex items-center gap-2 transition-all duration-300 text-white border bg-gray-500/20 border-gray-400/30 hover:bg-gray-500/40 hover:border-gray-400"
                    >
                 ...
                 <button 
                    ...
                    className="px-4 py-2 rounded-lg backdrop-blur-md shadow-lg flex items-center gap-2 transition-all duration-300 text-white border bg-indigo-500/20 border-indigo-400/30 hover:bg-indigo-500/40 hover:border-indigo-400"
                >
