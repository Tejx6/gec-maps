    ...
    return (
        <div ref={menuRef} className="fixed bottom-4 right-4 z-30">
            ...
            <button
                ...
                className={`w-16 h-16 rounded-full flex items-center justify-center transition-all duration-300 transform hover:scale-110 ${styles.button}`}
                aria-label="Profile"
            >
                <ProfileIcon className={`w-8 h-8 ${profileColor}`} />
            </button>
        </div>
    );
};
