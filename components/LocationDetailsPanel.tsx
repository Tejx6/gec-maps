...
const LocationDetailsPanel: React.FC<LocationDetailsPanelProps> = ({ location, onClearSelection, onStreetViewClick, onUploadClick, onEditClick, onAddReviewClick, user }) => {
    ...
    return (
        <div className={`absolute top-0 right-0 h-full w-96 transform transition-transform duration-500 ease-in-out ${location ? 'translate-x-0' : 'translate-x-full'}`}>
            <div 
                className="w-full h-full p-6 shadow-2xl text-gray-200 flex flex-col border-l border-white/10"
                    ...
                    <>
                        ...
                        <div className="flex-shrink-0">
                            <div className="flex items-start justify-between">
                                <h2 className="text-3xl font-bold text-cyan-300 drop-shadow-[0_0_8px_rgba(0,200,255,0.6)]">{location.name}</h2>
                        ...
                        {/* Content */}
                        <div className="flex-grow overflow-y-auto mt-4 p-4 rounded-lg bg-black/40 border border-white/10">
                            ...
                            {activeTab === 'faculty' && (
                             ...
                             {activeTab === 'reviews' && (
                                <div className="flex flex-col h-full">
                                    ...
                                    <button onClick={onAddReviewClick} className="mt-4 flex-shrink-0 w-full px-4 py-3 text-lg font-semibold text-cyan-300 uppercase transition-all duration-300 transform bg-cyan-400/10 border-2 border-cyan-400 rounded-lg shadow-lg hover:bg-cyan-400/20 hover:scale-105">
                                        Leave a Review
                                    </button>
                                </div>
                            )}
