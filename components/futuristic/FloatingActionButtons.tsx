...
const FloatingActionButtons: React.FC<FloatingActionButtonsProps> = ({ 
    ...
    const baseButtonClass = "relative w-14 h-14 rounded-full bg-black/30 backdrop-blur-md border border-white/10 flex items-center justify-center text-gray-300 transition-all duration-300 group shadow-lg";
    const hoverClass = "hover:border-cyan-400/50 hover:text-cyan-300 hover:scale-110";

    const Tooltip = ({ text }: { text: string }) => (
        <div className="absolute right-full mr-4 px-3 py-1.5 bg-gray-900/80 text-white text-sm rounded-md whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
            {text}
        </div>
    );
    
    return (
        <div className="absolute top-1/2 right-4 -translate-y-1/2 z-10 flex flex-col items-center gap-4">
            <div className="p-2 bg-black/20 backdrop-blur-lg rounded-full border border-white/10 flex flex-col gap-3">
                 <button onClick={onToggleViewMode} className={`${baseButtonClass} ${hoverClass}`}><SwitchViewIcon className="w-7 h-7" /><Tooltip text="Classic View" /></button>
                 <button onClick={onNavigateClick} className={`${baseButtonClass} ${hoverClass}`}><NavigateIcon className="w-7 h-7" /><Tooltip text="Navigate" /></button>
                 <button onClick={onStreetViewClick} className={`${baseButtonClass} ${hoverClass}`}><StreetViewIcon className="w-7 h-7" /><Tooltip text="Street View" /></button>
                 {user?.type === 'admin' && (
                    <>
                        <button onClick={onUploadClick} className={`${baseButtonClass} ${hoverClass}`}><UploadIcon className="w-7 h-7" /><Tooltip text="Upload 360°" /></button>
                        <button onClick={onSettingsClick} className={`${baseButtonClass} ${hoverClass}`}><SettingsIcon className="w-7 h-7" /><Tooltip text="Settings" /></button>
                    </>
                 )}
            </div>
            ...
            {user?.type === 'admin' && (
                <div className="flex flex-col gap-3">
                    <button 
                        ...
                        className={`relative w-16 h-16 rounded-full bg-gradient-to-br from-pink-500 to-red-600 flex items-center justify-center text-white shadow-lg shadow-pink-500/30 transition-all duration-300 transform hover:scale-110 ${isAddingEvent ? 'rotate-45' : ''}`}
                    ...
                    <button 
                        ...
                        className={`relative w-16 h-16 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white shadow-lg shadow-blue-500/30 transition-all duration-300 transform hover:scale-110 ${isAddingLocation ? 'rotate-45' : ''}`}
